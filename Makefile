...
pkgdatadir = $(datadir)/blockcoincore
pkgincludedir = $(includedir)/blockcoincore
pkglibdir = $(libdir)/blockcoincore
pkglibexecdir = $(libexecdir)/blockcoincore
...
CONFIG_HEADER = $(top_builddir)/src/config/blockcoin-config.h
CONFIG_CLEAN_FILES = libblockcoinconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libblockcoinconsensus.pc.in \
	...
	$(top_srcdir)/src/config/blockcoin-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/Blockcoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/Blockcoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/Blockcoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/Blockcoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = blockcoin-cli
BITCOIN_DAEMON_NAME = blockcoind
BITCOIN_GUI_NAME = blockcoin-qt
BITCOIN_TX_NAME = blockcoin-tx
...
COPYRIGHT_HOLDERS_FINAL = The Blockcoin Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Blockcoin Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/Blockcoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = blockcoincore
PACKAGE_BUGREPORT = https://github.com/blockcoinpay/blockcoin/issues
PACKAGE_NAME = Blockcoin Core
PACKAGE_STRING = Blockcoin Core 0.12.3
PACKAGE_TARNAME = blockcoincore
PACKAGE_URL = https://blockcoin.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/Blockcoin
abs_srcdir = /tmp/Blockcoin
abs_top_builddir = /tmp/Blockcoin
abs_top_srcdir = /tmp/Blockcoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/Blockcoin/build-aux/install-sh
...
pkgconfig_DATA = libblockcoinconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/blockcoin-cli.bash-completion \
	       $(top_srcdir)/contrib/blockcoin-tx.bash-completion \
	       $(top_srcdir)/contrib/blockcoind.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_blockcoin_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_blockcoin_coverage.info test_blockcoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Blockcoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/blockcoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/blockcoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/blockcoin-config.h
$(top_srcdir)/src/config/blockcoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/blockcoin-config.h src/config/stamp-h1
libblockcoinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libblockcoinconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Blockcoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Blockcoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Blockcoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_blockcoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_blockcoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_blockcoin_filtered.info: test_blockcoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_blockcoin_filtered.info
...
#test_blockcoin_coverage.info: baseline_filtered_combined.info test_blockcoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_blockcoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_blockcoin_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_blockcoin_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_blockcoin.coverage/.dirstamp:  test_blockcoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_blockcoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_blockcoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
