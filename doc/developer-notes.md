...
Blockcoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 4771 for connections and services them.
...
However, in open source software such as Blockcoin Core, where everyone uses
...
pay attention to for reviewers of Blockcoin Core code.

General Blockcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Blockcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Blockcoin Core so that they can be integrated
