...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in blockcoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("blockcoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Blockcoin features */
    { "blockcoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "blockcoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "blockcoin",               "gobject",                &gobject,                true,  {} },
    { "blockcoin",               "voteraw",                &voteraw,                true,  {} },
